...
//################################	Executing Function
	...
	function status(){
		...
		$totalTime = $creditTime + $currentTime;
		$currentTime = minuteTOtime($currentTime);
		$totalTime = minuteTOtime($totalTime);
		$creditTime = minuteTOtime($creditTime);
		$currentTime = "$currentTime[0] d : $currentTime[1] h : $currentTime[2] m";
		$totalTime = "$totalTime[0] d : $totalTime[1] h : $totalTime[2] m";
		$creditTime = "$creditTime[0] d : $creditTime[1] h : $creditTime[2] m";
		print(checkSlot($mac) . "," . $slotTime . "," . checkCredit() . "," . $currentTime . "," . $creditTime . "," . $totalTime . "," . checkIfLogged($mac));
	}
...
//################################	Details and Computations Function	
	...
	function checkCurrentTime($mac){
		$userTime = getDetailCurrentUser($mac)[0];
		$activeTime = getDetailActiveUser($mac)[0];
		if($userTime != "NULL"){
			if($activeTime != "NULL"){
				return mikrotikTOminute($activeTime['session-time-left']);
			}else{
				return mikrotikTOminute($userTime['limit-uptime']) - mikrotikTOminute($userTime['uptime']);
			}
		}else{
			return 0;
		}
	...
	function getDetailActiveUser($mac){//Done
		global $mip, $muser, $mpass, $API;
		if ($API->connect($mip, $muser, $mpass)) {
			$ARRAY = $API->comm("/ip/hotspot/active/print", array(
				"detail"=> "",
